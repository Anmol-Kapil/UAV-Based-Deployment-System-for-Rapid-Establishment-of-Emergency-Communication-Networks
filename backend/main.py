...
app = FastAPI(title="UAV Emergency Deployment GCS Backend", version="phase-5")
...
def api_status():
    return StatusResponse(
        ...
        uav_connected=False,  # honest: MAVLink connection arrives in Phase 6
...
# ---------------------------------------------------------------------------
# MAVLink/Gazebo-dependent endpoints — honest stubs (Phase 6+)
# ---------------------------------------------------------------------------
...
def api_mission_send():
    return NotImplementedResponse(
        phase_required=6,
        message="Sending a target to the UAV requires the MAVLink/SITL connection built in Phase 6.",
    )
...
def api_mission_abort():
    return NotImplementedResponse(
        phase_required=6,
        message="Abort/RTL requires the MAVLink connection built in Phase 6.",
    )
...
# ---------------------------------------------------------------------------
# Telemetry (Phase 7+) — WebSocket endpoint exists and is reachable now,
# but honestly reports that live telemetry isn't implemented rather than
# accepting a connection that never sends anything.
# ---------------------------------------------------------------------------
    ...
    await websocket.send_json(
        {
            "implemented": False,
            "phase_required": 7,
            "message": "Live telemetry streaming arrives in Phase 7.",
        }
    )
